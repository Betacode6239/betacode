...
export const EnquiryRouter = createTRPCRouter({
    create: publicProcedure.input(CreateEnquirySchema).mutation(async ({ctx, input}) =>{
        const isAlreadySubmitted = await ctx.prisma.enquiry.findFirst({
            where: {
                email: input.email,
                type: input.type
            }
        ...
        const createdEnquiry = await ctx.prisma.enquiry.create({
            data: input
        })
    ...
    delete: protectedProcedure.input(DeleteEnquirySchema).mutation(async ({ctx, input}) => {
        return await ctx.prisma.enquiry.delete({where: {id: input.id}})
    }),
    ...
    deleteMany: protectedProcedure.input(DeleteManyEnquirySchema).mutation(async ({ctx, input}) => {
        return await ctx.prisma.$transaction(input.ids.map(id => {
            return ctx.prisma.enquiry.delete({
                where: {
                    id
                }
            })
    ...
    getById: publicProcedure.input(GetEnquiryByIdSchema).query(async ({ctx, input}) => {
        return await ctx.prisma.enquiry.findFirst({where: {id: input.id}})
    }),
